module AuthenticatedSystem
  protected
    ...
    # Store the given <%= file_name %> id in the sessions.
    def current_<%= file_name %>=(new_<%= file_name %>)
      sessions[:<%= file_name %>_id] = new_<%= file_name %> ? new_<%= file_name %>.id : nil
    ...
    # Store the URI of the current request in the sessions.
    #
    # We can return to this location by calling #redirect_back_or_default.
    def store_location
      sessions[:return_to] = request.request_uri
    end
    ...
    # Called from #current_<%= file_name %>.  First attempt to login by the <%= file_name %> id stored in the sessions.
    def login_from_session
      self.current_<%= file_name %> = <%= class_name %>.find_by_id(sessions[:<%= file_name %>_id]) if session[:<%= file_name %>_id]
    end
    ...
    # This is ususally what you want; resetting the sessions willy-nilly wreaks
    # havoc with forgery protection, and is only strictly necessary on login.
    # However, **all sessions state variables should be unset here**.
    def logout_keeping_session!
      ...
      session[:<%= file_name %>_id] = nil   # keeps the sessions but kill our variable
      # explicitly kill any other sessions variables you set
    end

    # The sessions should only be reset at the tail end of a form POST --
